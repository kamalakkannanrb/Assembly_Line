...
export const CurrentItems=createContext<{ "Name": string,"ID":string,"QC": string}[]>([]);
export const SetCurrentItems=createContext<Dispatch<SetStateAction<{ "Name": string,"ID":string ,"QC": string }[]>>| null>(null);

export function ScannedItems({children}:{children:ReactNode}){
    const[data,setData]=useState<{ "Name": string,"ID":string,"QC": string}[]>([]);
    return(
